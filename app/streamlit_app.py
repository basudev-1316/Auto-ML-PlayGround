...
from src.experiment_tracking import EXPERIMENTS_PATH, generate_run_id, track_experiments
from src.explainability import explain_best_model
from src.logger import LOG_FILE, get_logger
...
from src.utils import build_versioned_model_paths, save_json_report
...
def run_training_workflow(dataset: pd.DataFrame, target_column: str) -> dict[str, object]:
    ...
    LOGGER.info(
        "Streamlit training workflow started. target_column=%s rows=%d columns=%d",
        target_column,
        len(dataset),
        len(dataset.columns),
    )
    ...
    experiments_path = track_experiments(run_id, raw_results, trained_models)
    ...
    record_prediction_event(best_model_name, len(predictions))
    LOGGER.info(
        "Predictions generated by best model '%s' for %d rows.",
        best_model_name,
        len(features),
    )
    ...
    versioned_model_path, versioned_metadata_path = build_versioned_model_paths(MODELS_DIR)
    model_bundle = {
    ...
    LOGGER.info("Saved versioned trained model bundle to %s", versioned_model_path)
    ...
    save_json_report(metadata_payload, versioned_metadata_path)
    LOGGER.info("Saved versioned model metadata to %s", versioned_metadata_path)
    ...
    save_json_report(
        {
            ...
            "metric_name": selection_results[best_model_name]["metric_name"],
            "reasoning": reasoning,
            "saved_model_path": str(versioned_model_path),
            "saved_model_metadata_path": str(versioned_metadata_path),
    ...
    return {
        ...
        "saved_model_path": versioned_model_path,
        "saved_model_metadata_path": versioned_metadata_path,
...
def main() -> None:
    ...
    try:
        ...
        record_failure(f"csv_read_failure: {exc}")
        st.error(f"Failed to read the uploaded CSV: {exc}")
        return

    if dataset.empty:
        record_failure("empty_dataset_uploaded")
    ...
    try:
        ...
        record_failure(f"problem_detection_warning: {exc}")
    ...
    if st.button("Train Model", use_container_width=True):
        try:
            ...
            LOGGER.info("Training completed successfully from Streamlit UI.")
        except Exception as exc:
            LOGGER.exception("Training failed in Streamlit UI.")
            record_failure(f"training_failure: {exc}")
        ...
        st.caption(f"Model metadata: {training_output['saved_model_metadata_path']}")
        st.caption(f"Experiment tracking: {training_output['experiments_path']}")
        st.caption(f"Run ID: {training_output['run_id']}")
        st.caption(f"Log file: {LOG_FILE}")
        st.caption(f"Monitoring metrics: {METRICS_PATH}")
